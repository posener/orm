type {{$.Type.PrefixPublic}}WhereBuilder struct {}

{{range $_, $f := .Type.Fields}}
// {{$.Type.PrefixPublic}}Where{{$f.Name}} adds a condition on {{$f.Name}} to the WHERE statement
func (*{{$.Type.PrefixPublic}}WhereBuilder) {{$f.Name}}(op common.Op, val {{$f.Type.ExtName $.Type.Package}}) common.Where {
...
func (*{{$.Type.PrefixPublic}}WhereBuilder) {{$f.Name}}In(vals ...{{$f.Type.ExtName $.Type.Package}}) common.Where {
...
func (*{{$.Type.PrefixPublic}}WhereBuilder) {{$f.Name}}Between(low, high {{$f.Type.ExtName $.Type.Package}}) common.Where {
