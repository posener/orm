...
// scan an SQL row to a AllCount struct
func (s *selector) scan(dialect string, rows *sql.Rows) (*AllCount, error) {
	switch dialect {
	case "mysql":
...
func (s *selector) scanmysql(rows *sql.Rows) (*AllCount, error) {
	var (
		vals = values(*rows)
		row  AllCount
		all  = s.selectAll()
	...
	if all || s.SelectVarCharByte {
		if vals[i] != nil {
			...
			case []byte:
				...
				row.VarCharByte = tmp
			default:
				return nil, fmt.Errorf(errMsg, "[]byte", i, vals[i], vals[i], "[]byte, []byte")
			}
		}
		i++
	}
...
func (s *selector) scansqlite3(rows *sql.Rows) (*AllCount, error) {
	var (
		vals = values(*rows)
		row  AllCount
		all  = s.selectAll()
	...
	if all || s.SelectVarCharByte {
		if vals[i] != nil {
			val, ok := vals[i].([]byte)
			if !ok {
				return nil, fmt.Errorf(errMsg, "[]byte", i, vals[i], vals[i], "[]byte")
			}
			tmp := []byte(val)
			row.VarCharByte = tmp
		}
