...
func personDB(t *testing.T, name string) porm.API {
	t.Helper()
	db, err := porm.New(name, conn(t, name))
	require.Nil(t, err)
	if testing.Verbose() {
		db.Logger(t.Logf)
	}
...
func allDB(t *testing.T, name string) aorm.API {
	t.Helper()
	db, err := aorm.New(name, conn(t, name))
	require.Nil(t, err)
	if testing.Verbose() {
		db.Logger(t.Logf)
	}
